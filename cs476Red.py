            ...
            if self.queue:
                ...
                if self.destination:
                    ...
                    if self.destination.isHost:
                        identity2 = "Host"
                    if debug: print(f"Link: \t\tSending {packet['type']} packet from {identity1} {self.source.id} to {identity2} {self.destination.id} (Destination: Host {packet["destination"].id})")
